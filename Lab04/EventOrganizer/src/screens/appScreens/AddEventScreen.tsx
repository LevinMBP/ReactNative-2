...
function AddEventScreen({ navigation }: { navigation: any }) {
    const { user } = useUser();
    const { control, handleSubmit, formState: { errors }, reset } = useForm<EventType>({
        defaultValues: {
            eventDate: dayjs().format("MMMM DD,YYYY"),
            eventName: "",
            eventPlace: "",
            eventStartTime: dayjs().format("HH:mm"),
            eventEndTime: dayjs().format("HH:mm"),
            eventDetails: "",
            favorites: []
        }
    })
    ...
    const onSubmit = async (data: EventType) => {
        ...
        try {
            const eventDoc = await addDoc(collection(FIREBASE_DB, 'events'), eventData);
            ...
            try {
                ...
                console.log("Updating Event Id error: ", error);
            }
            reset();
            Alert.alert("Event Posted", `Event Id:  ${eventDoc.id}`, [{
                text: "Ok",
                style: 'default',
